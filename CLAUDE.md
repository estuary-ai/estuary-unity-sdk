...
- voice_push_to_talk: Partial — PTT is LiveKit track mute/unmute only (`EstuaryMicrophone.HandleLiveKitPushToTalk` → `Unmute()`/`Mute()`); the gateway is never told about press/release and `turn_mode` is not declared, so the server-side PTT added in contract v1.11 (hold end-of-turn dispatch while the button is down; finalize nudge + grace flush on release) does not apply. A mid-hold pause lets semantic end-of-turn dispatch the turn (character talks over the held button), and release-time end-of-turn relies on VAD over a frame-less muted track. Adoption tracked in SCRUM-220; reference implementation is the Python SDK (`voice/livekit_voice.py`) per SDK_CONTRACT.md §voice_push_to_talk.
